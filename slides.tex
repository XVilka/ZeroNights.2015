...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
   \item \alert{r2 -A} или \alert{r2 + aaa} : Анализ
   \item \alert{s} : Переход по указанному адресу
   \item \alert{pdf} : Дизассемблирование функции
   \item \alert{af?} : Анализ функции
   \item \alert{ax?} : Анализ XREF
   \item \alert{/?} : Поиск
   \item \alert{ps?} : Напечатать строку (print string)
   \item \alert{C?} : Комментарии
   \item \alert{w?} : Запись (hex, опкодов, etc)
 \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{enumerate}
  \item \alert{V?} или просто \alert{?} : Помощь по командам
  \item \alert{p/P} : переключение между разными визуальными представлениями
  \item Навигация с помощью стрелок/hjkl
  \item \alert{o} : переместиться по адресу
  \item \alert{e} : визуальный режим настроек
  \item \alert{v} : список функций
  \item \alert{\_} : HUD
  \item \alert{V} : ASCII Graph
  \item \alert{0-9} : Прыжок на функцию
  \item \alert{u} : Undo
 \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Эмуляция участков кода}
     \begin{itemize}
        \item \alert{ae*} набор инструкций
		\item \alert{aei} - инициализация ESIL VM
		\item \alert{aeim} - инициализация стека/памяти VM
		\item \alert{aeip} - установка IP (Instruction Pointer)
		\item \alert{aes} - step в режиме эмуляции ESIL
		\item \alert{aec[u]} - continue [until]
		\item \alert{aef} - эмуляция функции
      \end{itemize}
\end{frame}
